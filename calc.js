...
let current_ids = [];
let selected_pickups = [];

function close_recipes() {
    document.getElementsByClassName("receipes_container")[0].style.display = "none";
    
}

function show_recipies(id) {
    //Remove precedent images if it exist
    var elementToDelete = document.getElementsByClassName("recipe_imgs");
    if (elementToDelete.length != 0) {
        document.getElementsByClassName("recipes_list")[0].innerHTML = "";
    }

    resultGrouped[id].forEach(recipe => {
        
        var newDiv = document.createElement('div');
        newDiv.setAttribute('class', 'recipe_imgs')
        document.getElementsByClassName("recipes_list")[0].appendChild(newDiv);

        recipe.forEach(recipe_item => {
            var newElement = document.createElement('img');
            newElement.setAttribute('class', 'recipe_img bofsym_' + recipe_item);
            newDiv.appendChild(newElement);
        })
    })


    document.getElementsByClassName("receipes_container")[0].style.display = "flex";
}

function combin(seq, k, seed) {
    var i = 0;
    var imax = (2**seq.length)-1;
    while (i <= imax) {
        var s = [];
        var j = 0;
        var jmax = seq.length - 1;
        while (j <= jmax){
            if ( (i>>j)&1==1 ) {
                s.push(seq[j]);
            }
        ...
        if (s.length == k) {
            var computedPool = current_pool.slice(0, 8 - k).concat(s);
            arrayresult.push([computedPool, get_result(computedPool, seed)]);
        }
        i += 1 
    }
}
...
function calculate_possibilities() {
    ...
    if (seed == "" || seed == "INSERT VALID SEED" || seed == 0) {
        document.getElementById('seed_input').value = "INSERT VALID SEED";
        return 0;
    }
    arrayresult = [];
    combin(selected_pickups, 8 - current_pool.filter(x => x != 0).length, seed);

    const setArray = new Set(arrayresult.map(x => JSON.stringify(x)));
    arrayresult = [...setArray].map(x => JSON.parse(x));
        
    resultGrouped = groupBy(arrayresult, 1);
    current_ids = Object.keys(resultGrouped);
...
function flush_result(){
    ...
    }else if (current_pool[0] != 0){
        /*combin(selected_pickups, 7, seed);
...
function select_pickup(id) {
    selected_pickups.push(id);

    var img = document.getElementById('add_btn_pickup_'+id);
    var numberHTML = img.parentElement.getElementsByTagName("span")[0].innerHTML;
    var number = parseInt(numberHTML);
    number += 1;
    img.parentElement.getElementsByTagName("span")[0].innerHTML = number + "";
    
    //CHANGE CLASS
    img.setAttribute('class', 'bofsym pickup-bofsym bofsym_' + id + ' selected_bofsym');
    img.parentElement.getElementsByTagName("span")[0].setAttribute('class', 'selected_number');
}
...
function remove_pickup(id) {
    ...
    if (!selected_pickups.includes(id)) {
        img.setAttribute('class', 'bofsym pickup-bofsym bofsym_' + id + ' unselected_bofsym');
        img.parentElement.getElementsByTagName("span")[0].setAttribute('class', 'unselected_number');
    }
}
...
for(let i=1;i<30;i++){
    ...
    img.parentElement.getElementsByClassName("plus_minus_overlay")[0].getElementsByTagName("div")[0].setAttribute('onclick', "remove_pickup(" + i + ")");

    img.parentElement.getElementsByClassName("plus_minus_overlay")[0].getElementsByTagName("div")[1].setAttribute('onclick', "select_pickup(" + i + ")");
